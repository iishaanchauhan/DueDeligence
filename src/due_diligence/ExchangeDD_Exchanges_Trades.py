...
def pull_trades_cexio(ccy_pair, end_time='2023-08-01T00:00:00', timeframe='1 hour'):
    """
    ...
    last_trade_cm = client.get_market_trades(
        cm_markets,
        start_time=(pd.to_datetime(end_time) - pd.Timedelta(timeframe)).isoformat(),
        limit_per_market=1,
        paging_from='start',
        start_inclusive=True).to_dataframe()
    last_cm_id = last_trade_cm.loc[last_trade_cm.index[-1], 'coin_metrics_id'] - 1
    output_list = []
    try:
        while True:
            ...
            cexio_df.rename(
                columns={'type': 'type', 'date': 'time', 'amount': 'amount', 'tid': 'UID'}, inplace=True)
            cexio_df['dtime'] = pd.to_datetime(cexio_df['time'].astype('int64'), unit='s')
...
def pull_trades_coinbase(ccy_pair, end_time='2023-08-01T00:00:00', timeframe='1 hour', client=client):
    """
    ...
    while True:
        try:
            url = f'https://api.exchange.coinbase.com/products/{input_ccy_pair}/trades?after={last_trade_id}' \
                  f'&limit={1000}'
            print(url)
            r = requests.get(url)
            output_sub_df = pd.DataFrame(r.json())
            output_sub_df.rename(columns={'trade_id': 'UID', 'side': 'type', 'size': 'amount', 'time': 'time'},
                                 inplace=True)
            output_sub_df['dtime'] = pd.to_datetime(output_sub_df['time']).dt.tz_localize(None)
...
def pull_trades_gateio(ccy_pair, end_time='2023-08-01T00:00:00', timeframe='1 hour'):
    """
    ...
    input_end_timestamp = (pd.to_datetime(end_time) - pd.Timedelta('1 day')).strftime('%Y%m')
    ...
    try:
        ...
        output_df['dtime'] = pd.to_datetime(output_df['time'].astype('float64'), unit='s').dt.tz_localize(None)
...
def pull_trades_cryptocom(ccy_pair, end_time='2023-08-01T00:00:00', timeframe='1 hour'):
    """
    ...
    input_start_time = (pd.to_datetime(end_time) - pd.Timedelta(timeframe)).value // 10 ** 6
    output_list = []
    try:
        while True:
            ...
            output_sub_df.rename(columns={'s': 'type', 'p': 'price', 'q': 'amount', 't': 'time', 'd': 'UID'},
                                 inplace=True)
            output_sub_df.drop(columns=['i'], inplace=True)
            output_sub_df['dtime'] = pd.to_datetime(output_sub_df['time'].astype('int64'), unit='ms')
            output_list.append(output_sub_df)
            input_end_time = int(output_sub_df.loc[output_sub_df.index[-1], 'time'])
            time.sleep(1)
            if input_end_time <= input_start_time:
                break
    except(ValueError, json.JSONDecodeError, requests.exceptions.HTTPError) as e:
        print(e)
    finally:
        output_df = pd.concat(output_list)
...
def pull_trades_bitflyer(ccy_pair, end_time='2023-08-01T00:00:00', timeframe='1 hour', client=client):
    """
    ...
    while True:
        try:
            ...
            output_sub_df.rename(
                columns={'id': 'UID', 'side': 'type', 'size': 'amount', 'exec_date': 'time'}, inplace=True)
            output_sub_df = output_sub_df[['UID', 'type', 'amount', 'price', 'time']]
            output_sub_df['dtime'] = pd.to_datetime(output_sub_df['time'], format='mixed')
            last_trade_id = output_sub_df.loc[output_sub_df.index[-1], 'UID']
            last_trade_timestamp = output_sub_df.loc[output_sub_df.index[-1], 'dtime'].tz_localize(None)
...
def pull_trades_bullish(ccy_pair, end_time='2023-08-01T00:00:00', timeframe='1 hour', client=client):
    """
    ...
    try:
        ...
        output_df = output_df[['UID', 'amount', 'price', 'type', 'time', 'dtime']]
        output_df['dtime'] = pd.to_datetime(output_df['dtime'], format='mixed').dt.tz_localize(None)
    except (json.JSONDecodeError, ValueError, requests.exceptions.HTTPError) as e:
        print(e)
    finally:
        output_df.insert(0, 'Currencypair', ccy_pair)
...
def pull_trades_itbit(ccy_pair, end_time='2023-08-01T00:00:00', timeframe='1 hour', client=client):
    """
    ...
    try:
        ...
        output_df.rename(columns={'match_number': 'UID', 'executed_at': 'time'}, inplace=True)
        output_df['dtime'] = pd.to_datetime(output_df['time'], format='mixed').dt.tz_localize(None)
    except (json.JSONDecodeError, ValueError, requests.exceptions.HTTPError) as e:
        print(e)
    finally:
        output_df.insert(0, 'Currencypair', ccy_pair)
...
def pull_trades_lbank(ccy_pair, end_time='2023-08-01T00:00:00', timeframe='1 hour', client=client):
    """
    ...
    while True:
        try:
            ...
            output_sub_df.rename(columns={'date_ms': 'time', 'tid': 'UID'}, inplace=True)
            output_sub_df['dtime'] = pd.to_datetime(output_sub_df['time'].astype('int64'), unit='ms')
            output_df_list.append(output_sub_df)
...
def pull_trades(currencypairs, exchanges, end_time='2022-08-01T00:00:00', timeframe='1 hour', to_csv=True,
    ...
    for currencypair in currencypairs:
        for exchange in exchanges:
            ...
            while tries <= 2:
                try:
                    func = f'pull_trades_{exchange}(ccy_pair="{currencypair}", end_time="{end_time}",' \
                    ...
                    print(f'Data request for exchange {exchange}, ticker {currencypair} done')
    ...
    if to_csv:
        output_path = (
                ...
                (pd.to_datetime(end_time) - pd.Timedelta(timeframe)).strftime('%Y-%m-%d'))
...
"""
df = pull_trades(
    ccy_pairs, exchanges_func_input, end_time=pull_date, timeframe='1 hour', to_csv=True,
    name_csv=f'ExchangeData_Trades_lbank_{csv_timestamp}')
