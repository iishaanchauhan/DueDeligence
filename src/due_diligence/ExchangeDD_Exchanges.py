...
def pull_data_kraken(ccy_pair, granul, pull_date='2022-08-01'):
    ...
    kraken_pull_date_start = (pd.to_datetime(pull_date) + pd.Timedelta('1 day')
                              - pd.Timedelta(granul * 720, 'h')).value // 10 ** 9
    api = krakenex.API()
    k = KrakenAPI(api)
    ...
    if traded_pairs[traded_pairs['altname'] == kraken_ccy_pair].shape[0] == 1:
        data_kraken = k.get_ohlc_data(kraken_ccy_pair, interval=60 * granul,
                                      since=kraken_pull_date_start)[0]  # 1440 for daily
        ...
        data_kraken = pd.DataFrame(columns=['time', 'open', 'high', 'low', 'close', 'volume'])
...
def pull_data_coinbase(ccy_pair, granul, pull_date):
    ...
    coinbase_pull_date_start = coinbase_pull_date_end - pd.to_timedelta(199 * granul, 'h')
    public_client = cbpro.PublicClient()
    coinbase_ccy_pair = ccy_pair.replace(':', '-')
    data = public_client.get_product_historic_rates(product_id=coinbase_ccy_pair,
                                                    granularity=3600 * granul,
                                                    start=coinbase_pull_date_start,
                                                    end=coinbase_pull_date_end)
    data_coinbase = pd.DataFrame(data, columns=['time', 'open', 'high', 'low', 'close', 'volume'])
    data_coinbase['dtime'] = pd.to_datetime(data_coinbase['time'].astype('int64'), unit='s')
...
def pull_data_bitfinex(ccy_pair, granul, pull_date):
    ...
    bitfinex_pull_date_end = (pd.to_datetime(pull_date) + pd.Timedelta('1 day')).value // 10 ** 6
    if granul == 1:
        tf = '1h'
    elif granul == 24:
        tf = '1D'
    ...
    if len(ccy_pair_list[0]) > 3:
        ...
        bitfinex_ccy_pair = ''.join(ccy_pair_list)
    try:
        bitfinex_pull_date_start = pd.to_datetime(pull_date) - pd.Timedelta(value=200, unit=tf[1])
        bitfinex_pull_date_start = bitfinex_pull_date_start.value // 10 ** 6
        url = (f"https://api.bitfinex.com/v2/candles/trade:{tf}:t{bitfinex_ccy_pair}/hist" +
               f"?start={bitfinex_pull_date_start}&end={bitfinex_pull_date_end}")
        print(url)
        r = requests.get(url, timeout=5)
        data = r.json()
        data_bitfinex = pd.DataFrame(data, columns=['time', 'open', 'close', 'high', 'low', 'volume'])
        ...
        data_bitfinex = data_bitfinex[['time', 'open', 'high', 'low', 'close', 'volume']]
        data_bitfinex = data_bitfinex.sort_index(ascending=True)
    except (json.decoder.JSONDecodeError, ValueError) as e:
        print(e)
        data_bitfinex = pd.DataFrame(columns=['time', 'open', 'high', 'low', 'close', 'volume'])
    finally:
...
def pull_data_bitstamp(ccy_pair, granul, pull_date):
    ...
    bitstamp_pull_date_end = (pd.to_datetime(pull_date) + pd.Timedelta('1 day')).value // 10 ** 9
    try:
        step = 3600 * granul
        ...
        print(e)
        data_bitstamp = pd.DataFrame(columns=['time', 'open', 'high', 'low', 'close', 'volume'])
    finally:
...
def pull_data_binance(ccy_pair, granul, pull_date):
    ...
    """
    client = BneClient()
    binance_pull_date_start = (pd.to_datetime(pull_date) - pd.Timedelta('30 days')).value // 10 ** 6
    interval = str(granul) + 'h'
    binance_ccy_pair = ccy_pair.replace(':', '')
    try:
        data_binance = pd.DataFrame(
            client.get_historical_klines(symbol=binance_ccy_pair, interval=interval, start_str=binance_pull_date_start))
        data_binance.columns = ['time', 'open', 'high', 'low', 'close', 'volume', 'close_time', 'qav', 'num_trades',
                                'taker_base_vol', 'taker_quote_vol', 'is_best_match']
        data_binance['time'] = data_binance['time'] / 1000
        data_binance['dtime'] = pd.to_datetime(data_binance['time'].astype('int64'), unit='s')
        data_binance.set_index('dtime', inplace=True)
        data_binance = data_binance[['time', 'open', 'high', 'low', 'close', 'volume']]
        ...
        print(e)
        data_binance = pd.DataFrame(columns=['time', 'open', 'high', 'low', 'close', 'volume'])
    finally:
...
def pull_data_bittrex(ccy_pair, granul, pull_date='2022-07-01'):
    ...
    :param ccy_pair: 
    :param granul: 
    :param pull_date: which date should the candles be pulled. Default to be 2022-08-01. 
    Must be the previous month at the latest. The API accepts only year or month internally
    depending on granul.
    :return: 
    """  #
    ...
    try:
        ...
        bittrex_df['dtime'] = pd.to_datetime(bittrex_df['time']).dt.tz_localize(None)
        ...
        print(e)
        bittrex_df = pd.DataFrame(columns=['time', 'open', 'high', 'low', 'close', 'volume'])
    finally:
...
def pull_data_bitbank(ccy_pair, granul, pull_date='2022-08-01'):
    ...
    if granul == 1:
        ...
        raise KeyError('invalid granul value, must be either 1 or 24')
    try:
        url = f'https://public.bitbank.cc/{bitbank_ccy_pair}/candlestick/{bitbank_granul}/{bitbank_pull_date_end}'
        print(url)
        r = requests.get(url, timeout=5)
        ...
        print(e)
        bitbank_df = pd.DataFrame(columns=['time', 'open', 'high', 'low', 'close', 'volume'])
    finally:
...
def pull_data_cexio(ccy_pair, granul, pull_date='2022-08-01'):
    ...
    if granul == 1:
        ...
        raise KeyError('invalid granul value, must be either 1 or 24')
    try:
        url = 'https://cex.io/api/ohlcv/hd/' + cexio_pull_date + '/' + cexio_ccy_pair
        print(url)
        ...
        print(e)
        cexio_df = pd.DataFrame(columns=['time', 'open', 'high', 'low', 'close', 'volume'])
    finally:
...
def pull_data_ftx(ccy_pair, granul, pull_date='2022-08-01'):
    ...
    ftx_pull_date_end = (pd.to_datetime(pull_date) + pd.Timedelta('1 day')).value // 10 ** 9
    ftx_pull_date_start = ftx_pull_date_end - 200 * ftx_granul
    url = (f'https://ftx.com/api/markets/{ftx_ccy_pair}/candles?resolution=' +
           f'{ftx_granul}&start_time={ftx_pull_date_start}&end_time={ftx_pull_date_end}')
    print(url)
    try:
        while True:
            r = requests.get(url, timeout=5)
        ...
        print(e)
        ftx_df = pd.DataFrame(columns=['time', 'open', 'high', 'low', 'close', 'volume'])
    finally:
...
def pull_data_gateio(ccy_pair, granul=1, pull_date='2022-06-30'):
    ...
    gateio_pull_date_end = (pd.to_datetime(pull_date) + pd.Timedelta('1 day')).value // 10 ** 9
    try:
        # Market candlesticks
        api_response = api_instance.list_candlesticks(gateio_ccy_pair, limit=200, to=gateio_pull_date_end,
                                                      interval=gateio_granul)
        gateio_df = pd.DataFrame(api_response,
                                 columns=['time', 'quotevolume', 'close', 'high', 'low', 'open', 'volume'])
        ...
        gateio_df = pd.DataFrame(columns=['time', 'open', 'high', 'low', 'close', 'volume'])
    finally:
...
def pull_data_gemini(ccy_pair, granul=1, pull_date='2022-08-01'):
    ...
    try:
        ...
        r = requests.get(url, timeout=5)
        print(url)
        gemini_df = pd.DataFrame(r.json(), columns=['time', 'open', 'high', 'low', 'close', 'volume'])
    ...
    except (requests.exceptions.RequestException, requests.exceptions.JSONDecodeError, KeyError, ValueError) as e:
        print(e)
        gemini_df = pd.DataFrame(columns=['time', 'open', 'high', 'low', 'close', 'volume'])
    finally:
...
def pull_data_poloniex(ccy_pair, granul=1, pull_date='2022-08-01'):
    ...
    try:
        url = (f'https://api.poloniex.com/markets/{poloniex_ccy_pair}/candles?'
               f'interval={poloniex_granul}&startTime={poloniex_pull_date_start}&limit=200')
        r = requests.get(url, timeout=5)
        print(url)
        poloniex_df = pd.DataFrame(r.json(),
                                   columns=['low', 'high', 'open', 'close', 'baseamount', 'volume',
                                            'buyTakerAmount', 'buyTakerQuantity', 'tradeCount', 'elapsed',
                                            'weightedavg',
                                            'interval', 'time', 'closeTime'])
        poloniex_df.drop(
            columns=['baseamount', 'buyTakerAmount', 'buyTakerQuantity', 'tradeCount', 'elapsed', 'weightedavg',
                     'interval', 'closeTime'],
            inplace=True)
        # check if we use start or end time to align with coinmetrics
        poloniex_df['dtime'] = pd.to_datetime(poloniex_df['time'].astype('int64'), unit='ms')
        poloniex_df.set_index('dtime', inplace=True)
        poloniex_df.sort_index(ascending=True, inplace=True)
    except (json.decoder.JSONDecodeError, ValueError, KeyError) as e:
        print(e)
        poloniex_df = pd.DataFrame(columns=['time', 'open', 'high', 'low', 'close', 'volume'])
    finally:
...
def pull_data_bibox(ccy_pair, granul, pull_date='2022-08-01'):
    ...
    url = f'https://api.bibox.com/v3/mdata/kline?pair={bibox_ccy_pair}&period={bibox_granul}'
    print(url)
    try:
        r = requests.get(url, timeout=5)
        bibox_df = pd.DataFrame(r.json()['result'])
        bibox_df['dtime'] = pd.to_datetime(bibox_df['time'].astype('int64'), unit='ms')
        ...
        print(e)
        bibox_df = pd.DataFrame(columns=['time', 'open', 'high', 'low', 'close', 'volume'])
    finally:
...
def pull_data_ftxus(ccy_pair, granul, pull_date='2022-08-01'):
    ...
    ftxus_pull_date_end = (pd.to_datetime(pull_date) + pd.Timedelta('1 day')).value // 10 ** 9
    ...
    url = (f'https://ftx.us/api/markets/{ftxus_ccy_pair}/candles?resolution=' +
           f'{ftxus_granul}&start_time={ftxus_pull_date_start}&end_time={ftxus_pull_date_end}')
    print(url)
    try:
        while True:
            r = requests.get(url, timeout=5)
        ...
        ftxus_df['dtime'] = pd.to_datetime(ftxus_df['time']).dt.tz_localize(None)
        ftxus_df.set_index('dtime', inplace=True)
        ftxus_df.sort_index(inplace=True, ascending=True)
    except (json.JSONDecodeError, ValueError, KeyError) as e:
        print(e)
        ftxus_df = pd.DataFrame(columns=['time', 'open', 'high', 'low', 'close', 'volume'])
    finally:
...
def pull_data_bitmex(ccy_pair, granul, pull_date='2022-08-01'):
    ...
    bitmex_pull_date_emd = (pd.to_datetime(pull_date) + pd.Timedelta('1 day')).strftime('%Y-%m-%d')
    url = (
        f'https://www.bitmex.com/api/v1/trade/bucketed?binSize={bitmex_granul}&partial=false&symbol={bitmex_ccy_pair}'
        f'&count={200}&reverse=false&startTime={pull_date}&endTime={bitmex_pull_date_emd}')
    print(url)
    try:
        r = requests.get(url, timeout=5)
        bitmex_df = pd.DataFrame(r.json())
        bitmex_df = bitmex_df[['open', 'high', 'low', 'close', 'volume', 'timestamp']]
        bitmex_df['dtime'] = pd.to_datetime(bitmex_df['timestamp']).dt.tz_localize(None)
        ...
        print(e)
        bitmex_df = pd.DataFrame(columns=['time', 'open', 'high', 'low', 'close', 'volume'])
    finally:
...
def pull_data_bybit(ccy_pair, granul, pull_date='2022-08-01'):
    ...
    bybit_pull_date_end = (pd.to_datetime(pull_date) + pd.Timedelta('1 day')).value // 10 ** 6
    ...
    try:
        ...
        bybit_df['dtime'] = pd.to_datetime(bybit_df['time'].astype('int64'), unit='ms')
        bybit_df.set_index('dtime', inplace=True)
        bybit_df.sort_index(ascending=True)
    except (json.JSONDecodeError, ValueError, KeyError, FailedRequestError, InvalidRequestError) as e:
        print(e)
        bybit_df = pd.DataFrame(columns=['time', 'open', 'high', 'low', 'close', 'volume'])
    finally:
...
def pull_data_cryptocom(ccy_pair, granul, pull_date='2022-08-01'):
    ...
    input_start_time = (pd.to_datetime(pull_date)- pd.Timedelta('1 day')).value // 10**6
    ...
    try:
        ...
        cryptocom_df.columns = ['open', 'high', 'low', 'close', 'volume', 'time']
        cryptocom_df['dtime'] = pd.to_datetime(cryptocom_df['time'].astype('int64'), unit='ms')
        cryptocom_df.set_index('dtime', inplace=True)
        cryptocom_df.sort_index(ascending=True, inplace=True)
    except (json.JSONDecodeError, ValueError, KeyError) as e:
        print(e)
        cryptocom_df = pd.DataFrame(columns=['time', 'open', 'high', 'low', 'close', 'volume'])
    finally:
...
def pull_data_hitbtc(ccy_pair, granul, pull_date='2022-08-01'):
    ...
    hitbtc_pull_date_end = (pd.to_datetime(pull_date) + pd.Timedelta('1 day')).strftime('%Y-%m-%d')
    url = (f'https://api.hitbtc.com/api/3/public/candles/'
           f'{hitbtc_ccy_pair}?sort=ASC&period={hitbtc_granul}&from={pull_date}&till={hitbtc_pull_date_end}&offset=0')
    print(url)
    try:
        r = requests.get(url, timeout=5)
        hitbtc_df = pd.DataFrame(r.json())
        hitbtc_df.columns = ['time', 'open', 'close', 'low', 'high', 'volume', 'basevolume']
        hitbtc_df.drop('basevolume', axis=1, inplace=True)
        hitbtc_df['dtime'] = pd.to_datetime(hitbtc_df['time']).dt.tz_localize(None)
        hitbtc_df.set_index('dtime', inplace=True)
        hitbtc_df.sort_index(ascending=True, inplace=True)
    except (json.JSONDecodeError, ValueError, KeyError) as e:
        print(e)
        hitbtc_df = pd.DataFrame(columns=['time', 'open', 'high', 'low', 'close', 'volume'])
    finally:
...
def pull_data_huobi(ccy_pair, granul, pull_date='2022-08-01'):
    ...
    try:
        ...
        huobi_df.rename(columns={'amount': 'volume', 'id': 'time'}, inplace=True)
        huobi_df['dtime'] = pd.to_datetime(huobi_df['time'].astype('int64'), unit='s')
        huobi_df.set_index('dtime', inplace=True)
        huobi_df.sort_index(ascending=True, inplace=True)
    except (json.JSONDecodeError, ValueError, KeyError) as e:
        print(e)
        huobi_df = pd.DataFrame(columns=['time', 'open', 'high', 'low', 'close', 'volume'])
    finally:
...
def pull_data_kucoin(ccy_pair, granul, pull_date='2022-08-01'):
    ...
    kucoin_pull_date_end = (pd.to_datetime(pull_date) + pd.Timedelta('1 day')).value // 10 ** 9
    kucoin_pull_date_start = pd.to_datetime(pull_date).value // 10 ** 9
    client = KcAPI(url='https://api.kucoin.com')
    try:
        kucoin_df = pd.DataFrame(
            client.get_kline(kline_type=kucoin_granul, symbol=kucoin_ccy_pair, startAt=kucoin_pull_date_start,
                             endAt=kucoin_pull_date_end),
            columns=['time', 'open', 'close', 'high', 'low', 'volume', 'basevolume'])
        kucoin_df.drop(['basevolume'], axis=1, inplace=True)
        kucoin_df['dtime'] = pd.to_datetime(kucoin_df['time'].astype('int64'), unit='s')
        kucoin_df.set_index('dtime', inplace=True)
        kucoin_df.sort_index(ascending=True, inplace=True)
    except (json.JSONDecodeError, ValueError, KeyError, Exception) as e:
        print(e)
        kucoin_df = pd.DataFrame(columns=['time', 'open', 'high', 'low', 'close', 'volume'])
    finally:
...
def pull_data_lbank(ccy_pair, granul, pull_date='2022-08-01'):
    ...
    # lbank_pull_date_end = (pd.to_datetime(pull_date) + pd.Timedelta('1 day')).value // 10 ** 9
    lbank_pull_date_start = pd.to_datetime(pull_date).value // 10 ** 9
    url = (f'https://api.lbkex.com/v2/kline.do'
           f'?symbol={lbank_ccy_pair}&size=200&type={lbank_granul}&time={lbank_pull_date_start}')
    print(url)
    try:
        r = requests.get(url, timeout=5)
        lbank_df = pd.DataFrame(r.json()['data'])
        lbank_df.columns = ['time', 'open', 'high', 'low', 'close', 'volume']
        lbank_df['dtime'] = pd.to_datetime(lbank_df['time'].astype('int64'), unit='s')
        lbank_df.set_index('dtime', inplace=True)
        lbank_df.sort_index(ascending=True, inplace=True)
    except (json.JSONDecodeError, KeyError, ValueError) as e:
        print(e)
        lbank_df = pd.DataFrame(columns=['time', 'open', 'high', 'low', 'close', 'volume'])
    finally:
...
def pull_data_liquid(ccy_pair, granul, pull_date='2022-08-01'):
    ...
    try:
        liquid_pairs_url = pd.DataFrame(requests.get('https://api.liquid.com/products').json())
        liquid_product_id = (liquid_pairs_url.loc[
            liquid_pairs_url['currency_pair_code'] == liquid_ccy_pair,
            'id'].values[0])
        url = (f'https://api.liquid.com/products/{liquid_product_id}/ohlc?resolution={liquid_granul}')
        print(url)
        r = requests.get(url, timeout=5)
        liquid_df = pd.DataFrame(r.json()['data'])
        liquid_df.columns = ['time', 'open', 'high', 'low', 'close', 'volume']
        liquid_df['dtime'] = pd.to_datetime(liquid_df['time'].astype('int64'), unit='s')
        liquid_df.set_index('dtime', inplace=True)
        liquid_df.sort_index(ascending=True, inplace=True)
    except (json.JSONDecodeError, KeyError, ValueError, IndexError) as e:
        print(e)
        liquid_df = pd.DataFrame(columns=['time', 'open', 'high', 'low', 'close', 'volume'])
    finally:
...
def pull_data_okex(ccy_pair, granul, pull_date='2022-08-01'):
    ...
    okex_pull_date_end = (pd.to_datetime(pull_date) + pd.Timedelta('1 day')).value // 10 ** 9
    okex_pull_date_start = pd.to_datetime(pull_date).value // 10 ** 9
    url = (f'https://aws.okx.com/api/v5/market/candles'
           f'?instId={okex_ccy_pair}&bar={okex_granul}&before={okex_pull_date_end}&limit=200')
    print(url)
    try:
        r = requests.get(url, timeout=5)
        okex_df = pd.DataFrame(r.json()['data'])
        okex_df.columns = ['time', 'open', 'high', 'low', 'close', 'volume', 'ccyvol', 'quoteccyvol', 'confirm']
        okex_df.drop(columns=['ccyvol', 'quoteccyvol', 'confirm'], inplace=True)
        okex_df['dtime'] = pd.to_datetime(okex_df['time'].astype('int64'), unit='ms')
        okex_df.set_index('dtime', inplace=True)
        okex_df.sort_index(ascending=True, inplace=True)
    except (json.JSONDecodeError, KeyError, ValueError) as e:
        print(e)
        okex_df = pd.DataFrame(columns=['time', 'open', 'high', 'low', 'close', 'volume'])
    finally:
...
def pull_data_therocktrading(ccy_pair, granul, pull_date='2022-08-01'):
    ...
    therocktrading_pull_date_end = (pd.to_datetime(pull_date) + pd.Timedelta('1 day')).isoformat() + 'Z'
    therocktrading_pull_date_start = pd.to_datetime(pull_date).value // 10 ** 9
    url = (f'https://api.therocktrading.com/v1/funds/{therocktrading_ccy_pair}/ohlc_statistics'
           f'?period={therocktrading_granul}&before={therocktrading_pull_date_end}&sort=ASC')
    print(url)
    try:
        r = requests.get(url, timeout=5)
        therocktrading_df = pd.DataFrame(r.json())
        therocktrading_df = therocktrading_df[['open', 'high', 'low', 'close', 'traded_volume', 'interval_starts_at']]
        therocktrading_df.rename(columns={'interval_starts_at': 'time', 'traded_volume': 'volume'}, inplace=True)
        therocktrading_df['dtime'] = pd.to_datetime(therocktrading_df['time']).dt.tz_localize(None)
        therocktrading_df.set_index('dtime', inplace=True)
        therocktrading_df.sort_index(ascending=True, inplace=True)
    except (json.JSONDecodeError, KeyError, ValueError) as e:
        print(e)
        therocktrading_df = pd.DataFrame(columns=['time', 'open', 'high', 'low', 'close', 'volume'])
    finally:
...
def pull_data_zbcom(ccy_pair, granul, pull_date='2022-08-01'):
    ...
    zbcom_pull_date_start = (pd.to_datetime(pull_date) - pd.Timedelta('30 days')).value // 10 ** 6
    url = (f'https://api.zb.com/data/v1/kline'
           f'?market={zbcom_ccy_pair}&since={zbcom_pull_date_start}&type={zbcom_granul}')
    print(url)
    try:
        r = requests.get(url, timeout=5)
        zbcom_df = pd.DataFrame(r.json()['data'])
        zbcom_df.columns = ['time', 'open', 'high', 'low', 'close', 'volume']
        zbcom_df['dtime'] = pd.to_datetime(zbcom_df['time'].astype('int64'), unit='ms')
        zbcom_df.set_index('dtime', inplace=True)
        zbcom_df.sort_index(ascending=True, inplace=True)
    except (json.JSONDecodeError, KeyError, ValueError) as e:
        print(e)
        zbcom_df = pd.DataFrame(columns=['time', 'open', 'high', 'low', 'close', 'volume'])
    finally:
...
def pull_data_bithumb(ccy_pair, granul, pull_date='2022-08-01'):
    ...
    url = f'https://api.bithumb.com/public/candlestick/{bithumb_ccy_pair}/{bithumb_granul}'
    print(url)
    try:
        r = requests.get(url, timeout=5)
        bithumb_df = pd.DataFrame(r.json()['data'])
        bithumb_df.columns = ['time', 'close', 'open', 'high', 'low', 'volume']
        bithumb_df['dtime'] = pd.to_datetime(bithumb_df['time'].astype('int64'), unit='ms')
        bithumb_df.set_index('dtime', inplace=True)
        bithumb_df.sort_index(ascending=True, inplace=True)
    except (json.JSONDecodeError, KeyError, ValueError) as e:
        print(e)
        bithumb_df = pd.DataFrame(columns=['time', 'open', 'high', 'low', 'close', 'volume'])
    finally:
...
def pull_data_upbit(ccy_pair, granul, pull_date='2022-08-01'):
    ...
    upbit_pull_date_end = (pd.to_datetime(pull_date) + pd.Timedelta('1 day')).strftime('%Y-%m-%dT%H:%M:%SZ')
    ...
    try:
        ...
        upbit_df = upbit_df[[
            'candle_date_time_utc', 'trade_price', 'opening_price', 'high_price', 'low_price',
        ...
        print(e)
        upbit_df = pd.DataFrame(columns=['time', 'open', 'high', 'low', 'close', 'volume'])
    finally:
...
def pull_data_mexc(ccy_pair, granul, pull_date='2023-03-31'):
    ...
    mexc_pull_date_end = (pd.to_datetime(pull_date) + pd.Timedelta('1 day')).value // 10 ** 6
    mrkt_url = 'https://api.mexc.com/api/v3/klines'
    try:
        r = requests.get(url=mrkt_url,
                         params={'symbol': mexc_CCY_pair, 'interval': mexc_granul, 'endTime': mexc_pull_date_end,
                                 'startTime': mexc_pull_date_start})
        r_json = r.json()
        data_mexc = pd.DataFrame(r_json)
        data_mexc.columns = ['time', 'open', 'close', 'high', 'low', 'volume', 'close time', 'quote volume']
        # actual order: open high low close
        data_mexc = data_mexc[['time', 'open', 'close', 'high', 'low', 'volume']]
        ...
        print(e)
        data_mexc = pd.DataFrame(columns=['time', 'open', 'high', 'low', 'close', 'volume'])
    finally:
...
def pull_data_bullish(ccy_pair, granul, pull_date='2023-03-31'):
    ...
    if granul == 1:
        ...
        bullish_granul = '1d'
    bullish_pull_date_start = pd.to_datetime(pull_date).isoformat(timespec='milliseconds') + 'Z'
    bullish_pull_date_end = (pd.to_datetime(pull_date) + pd.Timedelta('1 day')).isoformat(timespec='milliseconds') + 'Z'
    mrkt_url = 'https://api.exchange.bullish.com/trading-api/v1/markets/' + bullish_ccy_pair + '/candle'
    try:
        responses = requests.get(url=mrkt_url,
                                 params={'createdAtDatetime[gte]': bullish_pull_date_start,
                                         'createdAtDatetime[lte]': bullish_pull_date_end,
                                         'timeBucket': bullish_granul})
        print(responses.url)
        data_bullish = pd.DataFrame(responses.json())
        # data_bullish.columns = ['open', 'dtime', 'time', 'high', 'low', 'close', 'volume']
        data_bullish.rename(columns={'createdAtDatetime': 'time', 'createdAtTimestamp': 'dtime'}, inplace=True)
        ...
        print(e)
        data_bullish = pd.DataFrame(columns=['time', 'open', 'high', 'low', 'close', 'volume'])
    finally:
        data_bullish.insert(0, 'Currencypair', ccy_pair)
    return data_bullish


def pull_data(currencypairs, granul, exchanges, pull_date, to_csv=True, name_csv='ExchangesData'):
    """
    ...
    # Pulling data very every ccypair and exchange and concatenating into DF by exchange
    for currencypair in currencypairs:
        for exchange in exchanges:
            tries = 0
            while tries <= 2:
                try:
                    func = f'pull_data_{exchange}(ccy_pair="{currencypair}", granul={granul}, pull_date="{pull_date}")'
                    print(func)
                    exchange_ccy_df = eval(func)
                    exchange_ccy_df['exchange'] = exchange
                    exchange_df.append(exchange_ccy_df)
                    print(exchange_ccy_df.head())
                    print(f'Data request for exchange {exchange}, ticker {currencypair} on {pull_date} done')
    ...
    if to_csv:
        output_path = Path(__file__).parents[2] / 'output' / 'ExchangeDD' / pull_date
        if not output_path.exists():
            output_path.mkdir()
        output_file = (output_path
                       / f'{name_csv}_{pull_date.replace("-", "")}_{pd.Timestamp.today().strftime("%Y%m%d")}.csv')
...
exchanges = ['binance', 'binance.us', 'bitbank', 'bitfinex', 'bitflyer', 'bitstamp', 'bittrex', 'cex.io', 'coinbase',
             'ftx', 'gate.io', 'gemini', 'itbit', 'kraken', 'poloniex', 'ftx.us', 'bibox', 'bitmex', 'bybit',
             'crypto.com', 'hitbtc', 'huobi', 'kucoin', 'lbank', 'liquid', 'okex', 'therocktrading', 'zb.com',
             'bithumb', 'upbit', 'mexc', 'bullish']
exchanges_func_input = ['binance', 'binanceus', 'bitbank', 'bitfinex', 'bitstamp', 'bittrex', 'cexio', 'coinbase',
                        'ftx', 'gateio', 'gemini', 'kraken', 'poloniex', 'ftxus', 'bibox', 'bitmex', 'bybit',
                        'cryptocom', 'hitbtc', 'huobi', 'kucoin', 'lbank', 'liquid', 'okex', 'therocktrading', 'zbcom',
                        'bithumb', 'upbit', 'mexc', 'bullish']
ccy_pairs = ['ADA:USD', 'AVAX:USD', 'BTC:USD', 'ETH:USD', 'XRP:USD', 'USDT:USD', 'USDC:USD',
             'BNB:USD', 'BUSD:USD', 'SOL:USD', 'DOGE:USD', 'CRV:USD', 'BTC:JPY', 'ETH:JPY', 'CRV:JPY', 'DOGE:JPY',
             'XRP:JPY', 'USDT:JPY', 'SOL:JPY', 'BTC:EUR', 'ETH:EUR', 'SOL:EUR', '1INCH:USD', 'UNI:USD',
             'BCH:USD', 'BTC:USDT', '1INCH:USDT', 'MONA:JPY', '1INCH:EUR', 'ADA:EUR', 'ENJ:JPY',
             'UNI:USDT', 'BCH:JPY', 'LTC:USDT', 'BNB:USD'
             ]
print(Path.cwd())
...
Execution
"""
df = pull_data(ccy_pairs, granularity, exchanges_func_input, pull_date_global, to_csv=True,
