...
print(val_date)
markets_all = scope_check_trades(client_key=client, exchanges=exchanges, val_date=val_date,
                                 quote_ccys=quote_ccys, granul='1h', lookback_period=1000)
markets = pd.merge(left=markets_all, right=assets_df['asset'], left_on='base', right_on='asset', how='inner')

output_path = Path(__file__).parents[2] / 'output' / 'ExchangeDD' / ref_date / 'Coin Metrics Trades'

output_data = (output_path
               / f'CoinMetricsTrades_bittrex_{val_time_end.replace("-", "").replace(":", "")}'
                 f'_{pd.Timestamp.today().strftime("%Y%m%d")}.csv'
               )
output_scope = (output_path
                / f'CoinMetricsTrades_bittrex_markets_{val_time_end.replace("-", "").replace(":", "")}'
                  f'_{pd.Timestamp.today().strftime("%Y%m%d")}.csv')
# data extraction
...
for chunk in np.array_split(markets.index, markets.shape[0] // 3 + 1):
    try:
        ...
        print(f'{trades_df.shape[0]} trades for {markets.loc[chunk, "market"].to_list()}'
