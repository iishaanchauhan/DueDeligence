...
def scope_check_iCAVE(val_date, exchange_input, ccy_df, crypto_input, client,
                      lookback_period=10):
    """
    Returns all crypto-fiat markets among reliable exchanges either as csv
    export and/or as dataframe output.

    :param val_date: the valuation date
    :param ccy_df: the static dataframe of all fiat currencies
    :param crypto_input: the static dataframe of crypto assets with entry and
        exit dates
    ...
    :return: a table of all crypto-fiat markets with available minutia data in
        CoinMetrics with optional constraints on exchanges, currencies and
        crypto assets.
...
for val_date in val_dates:
    fiat_markets = scope_check_iCAVE(
        val_date=val_date, exchange_input=exchanges_df,
    ...
    if vol_stat and not output_file_stats.exists():
        hist_vol = list()
        for chunk in np.array_split(fiat_markets.index,
                                    fiat_markets.shape[0] // 20 + 1):
        ...
        hist_vol[['exchange', 'base', 'quote', 'spot']] = hist_vol[
            'market'].str.split('-', expand=True)
        hist_vol = hist_vol.merge(fiat_markets[['market', 'full_name']],
                                  on='market', how='inner')
        hist_vol.to_csv(
            os.path.join(
                val_date,
