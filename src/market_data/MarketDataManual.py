...
std_coverage = pd.read_csv(Path(__file__).parents[1] / 'static' / 'iCAVE default coverage.csv')
...
for val_date in val_dates:
    ...
    if not output_path.exists() or not input_scope.exists():
        ...
        print(f'please execute ScopeCheckManual.py to determine scope for market data extraction as of {val_date}')
    else:
        output_pricing = (
                output_path /
                f'CoinMetricsData_manual_{val_date.replace("-", "")}_{pd.Timestamp.today().strftime("%Y%m%d")}'
                f'.csv'
        )
    ...
    df = get_markets_data(pricing_coverage, val_date=val_date, client_key=client, granul=granul,
    ...
    if input_conversion.exists():
        output_conversion = (
                output_path /
                f'CoinMetricsData_manualconv_{val_date.replace("-", "")}_{pd.Timestamp.today().strftime("%Y%m%d")}'
                f'.csv'
        )
        conversion_coverage = pd.read_csv(input_conversion)
        print(f'number of conversion quotes: {conversion_coverage.shape[0]}')
        df = get_markets_data(conversion_coverage, val_date=val_date, client_key=client, granul=granul,
                              lookback_price=0, lookback_volume=10, vol_hist=True)
        df.to_csv(output_conversion, index=False)
    else:
        print(
            'if the data extraction is intended for manual assessment, '
            'please ensure that the flag crypto_only is True when running ScopeCheckManual.py')
